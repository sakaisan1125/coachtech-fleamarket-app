@extends('layouts.app')

@section('css')
    <link rel="stylesheet" href="{{ asset('css/mypage.css') }}">
@endsection

@section('content')
<div class="mypage-container">
    <div class="profile-area">
        <div class="profile-icon">
            @if (Auth::user()->profile_image)
                <img 
                    src="{{ \Illuminate\Support\Facades\Storage::url(Auth::user()->profile_image) }}" 
                    alt="プロフィール画像" 
                    class="profile-avatar"
                >
            @endif
        </div>
        <div class="profile-info">
            <div class="profile-username">
                {{ Auth::user()->name ?? 'ユーザー名' }}
            </div>
            @if (!is_null($ratingRounded) && $ratingCount > 0)
                <div 
                    class="rating-stars" 
                    aria-label="平均評価 {{ $ratingRounded }} / 5（{{ $ratingCount }}件）"
                >
                    @for ($i = 1; $i <= 5; $i++)
                        <span class="star {{ $i <= $ratingRounded ? 'star-filled' : '' }}">
                            {{ $i <= $ratingRounded ? '★' : '☆' }}
                        </span>
                    @endfor
                </div>
            @endif
        </div>
        <a 
            href="{{ route('mypage.profile.edit') }}" 
            class="profile-edit-btn"
        >
            プロフィールを編集
        </a>
    </div>

    <div class="mypage-tab-container">
        <div class="mypage-tab-menu">
            <a 
                href="{{ route('mypage.index') }}" 
                class="mypage-tab {{ $page === 'sell' ? 'active' : '' }}"
            >
                出品した商品
            </a>
            <a 
                href="{{ route('mypage.index', ['page' => 'buy']) }}" 
                class="mypage-tab {{ $page === 'buy' ? 'active' : '' }}"
            >
                購入した商品
            </a>
            <a 
                href="{{ route('mypage.index', ['page' => 'transactions']) }}" 
                class="mypage-tab {{ $page === 'transactions' ? 'active' : '' }}"
            >
                取引中の商品
                @if ($totalMessageCount > 0)
                    <span class="notification-badge">{{ $totalMessageCount }}</span>
                @endif
            </a>
        </div>
    </div>

    <div class="item-list">
        @forelse ($items as $item)
            <div class="item-card">
                <a 
                    href="{{ $page === 'transactions' 
                        ? route('chat.index', ['purchaseId' => $item->id]) 
                        : route('items.show', $item->id) 
                    }}" 
                    class="item-link"
                >
                    <div class="item-image-placeholder">
                        @if ($page === 'transactions' && ($messageCounts[$item->id] ?? 0) > 0)
                            <span class="message-count-badge">
                                {{ $messageCounts[$item->id] }}
                            </span>
                        @endif
                        @if ($item->image_url ?? optional($item->item)->image_url)
                            <img 
                                src="{{ $item->image_url ?? optional($item->item)->image_url }}" 
                                alt="商品画像" 
                                class="item-image"
                            >
                        @else
                            <span class="item-image-text">商品画像</span>
                        @endif
                        @if (!empty($item->is_sold) && $item->is_sold)
                            <span class="sold-badge">SOLD</span>
                        @endif
                    </div>
                    <div class="item-name">
                        {{ $item->name ?? optional($item->item)->name }}
                    </div>
                </a>
            </div>
        @empty
            <div class="no-items-message">
                @if ($page === 'buy')
                    購入した商品はありません。
                @elseif ($page === 'transactions')
                    取引中の商品はありません。
                @else
                    出品した商品はありません。
                @endif
            </div>
        @endforelse
    </div>
</div>
@endsection